...
q_va_jitter <- ggplot(data = rwd, aes(x=volatile.acidity, y=quality)) + 
  geom_point(position = 'jitter')

q_va_box <- ggplot(data = rwd, aes(x = quality, y = volatile.acidity)) +
  ...
  ggtitle('median trend')

q_va_den <- ggplot(rwd, aes(fixed.acidity, color=quality_fa)) +
  geom_density()

plot_grid(q_va_jitter, q_va_den, q_va_box, ncol=1)
```
...
```{r fig.height=8, fig.align='center', echo=FALSE, message=FALSE}
q_ac_jitter <- ggplot(data=rwd, aes(x=alcohol, y=quality)) + 
  geom_jitter()

q_ac_den <- ggplot(rwd, aes(alcohol, color=quality_fa)) +
  geom_density()

q_ac_box <- ggplot(data = rwd, aes(x = quality, y = alcohol)) +
  geom_boxplot(aes(group=quality)) + 
  stat_summary(fun.y=median, color="red", geom="line", size = 1) + 
  stat_summary(fun.y = "median", color = "red", size = 3, geom = "point") + 
  ggtitle('median trend')

plot_grid(q_ac_jitter, q_ac_den, q_ac_box, ncol=1)
```
...
```{r fig.height=8, fig.align='center', echo=FALSE, message=FALSE}
q_ca_jitter <- ggplot(data=rwd, aes(x=citric.acid, y=quality)) + 
  geom_point(position = 'jitter')

q_ca_den <- ggplot(rwd, aes(citric.acid, color=quality_fa)) +
  geom_density()

q_ca_box <- ggplot(data = rwd, aes(x = quality, y = citric.acid)) +
  geom_boxplot(aes(group=quality)) + 
  stat_summary(fun.y=median, color="red", geom="line", size = 1) + 
  stat_summary(fun.y = "median", color = "red", size = 3, geom = "point") + 
  ggtitle('median trend')

plot_grid(q_ca_jitter, q_ca_den, q_ca_box, ncol=1)
```
...
```{r fig.height=8, fig.align='center', echo=FALSE, message=FALSE}
q_su_jitter <- ggplot(data=rwd, aes(x=sulphates, y=quality)) + 
  geom_point(position = 'jitter')

q_su_den <- ggplot(rwd, aes(sulphates, color=quality_fa)) +
  geom_density()

q_su_box <- ggplot(data = rwd, aes(x = quality, y = sulphates)) +
  geom_boxplot(aes(group=quality)) + 
  stat_summary(fun.y=median, color="red", geom="line", size = 1) + 
  stat_summary(fun.y = "median", color = "red", size = 3, geom = "point") + 
  ggtitle('median trend')

plot_grid(q_su_jitter, q_su_den, q_su_box, ncol=1)
```
...
```{r fig.height=5, fig.align='center', echo=FALSE, message=FALSE}
ggplot(data=rwd, aes(x=sulphates, y=quality)) + 
  geom_point(position = 'jitter') + 
...
```{r fig.height=6, fig.width=10, fig.align='center', echo=FALSE, message=FALSE, warning=FALSE}
q_fa_jitter <- ggplot(data=rwd, aes(x=fixed.acidity, y=quality)) + 
  geom_point(position = 'jitter') 
q_fa_box <- ggplot(data = rwd, aes(x = quality, y = fixed.acidity)) +
  geom_boxplot(aes(group=quality)) + 
  scale_y_continuous(limits = c(quantile(rwd$fixed.acidity, 0.05), quantile(rwd$fixed.acidity, 0.95))) + 
  stat_summary(fun.y=median, color="red", geom="line", size = 1) + 
  stat_summary(fun.y = "median", color = "red", size = 3, geom = "point") + 
  ggtitle('median trend')

q_rs_jitter <- ggplot(data=rwd, aes(x=residual.sugar, y=quality)) + 
  geom_point(position = 'jitter') 
q_rs_box <- ggplot(data = rwd, aes(x = quality, y = residual.sugar)) +
  geom_boxplot(aes(group=quality)) + 
  scale_y_continuous(limits = c(quantile(rwd$residual.sugar, 0.05), quantile(rwd$residual.sugar, 0.95))) + 
  stat_summary(fun.y=median, color="red", geom="line", size = 1) + 
  stat_summary(fun.y = "median", color = "red", size = 3, geom = "point") + 
  ggtitle('median trend')

plot_grid(q_fa_jitter, q_fa_box, q_rs_jitter, q_rs_box, ncol=2)
```
...
```{r fig.height=6, fig.width=10, fig.align='center', echo=FALSE, message=FALSE, warning=FALSE}
q_fsd_jitter <- ggplot(data=rwd, aes(x=free.sulfur.dioxide, y=quality)) + 
  geom_point(position = 'jitter') 
q_fsd_box <- ggplot(data = rwd, aes(x = quality, y = free.sulfur.dioxide)) +
  geom_boxplot(aes(group=quality)) + 
  scale_y_continuous(limits = c(quantile(rwd$free.sulfur.dioxide, 0.05), quantile(rwd$free.sulfur.dioxide, 0.95))) + 
  stat_summary(fun.y=median, color="red", geom="line", size = 1) + 
  stat_summary(fun.y = "median", color = "red", size = 3, geom = "point") + 
  ggtitle('median trend')

q_tsd_jitter <- ggplot(data=rwd, aes(x=total.sulfur.dioxide, y=quality)) + 
  geom_point(position = 'jitter') 
q_tsd_box <- ggplot(data = rwd, aes(x = quality, y = total.sulfur.dioxide)) +
  geom_boxplot(aes(group=quality)) + 
  scale_y_continuous(limits = c(quantile(rwd$total.sulfur.dioxide, 0.05), quantile(rwd$total.sulfur.dioxide, 0.95))) + 
  stat_summary(fun.y=median, color="red", geom="line", size = 1) + 
  stat_summary(fun.y = "median", color = "red", size = 3, geom = "point") + 
  ggtitle('median trend')

plot_grid(q_fsd_jitter, q_fsd_box, q_tsd_jitter, q_tsd_box, ncol=2)
```
...
```{r fig.height=6, fig.width=10, fig.align='center', echo=FALSE, message=FALSE, warning=FALSE}
q_den_jitter <- ggplot(data=rwd, aes(x=density, y=quality)) + 
  geom_point(position = 'jitter') 
q_den_box <- ggplot(data = rwd, aes(x = quality, y = density)) +
  geom_boxplot(aes(group=quality)) + 
  scale_y_continuous(limits = c(quantile(rwd$density, 0.05), quantile(rwd$density, 0.95))) + 
  stat_summary(fun.y=median, color="red", geom="line", size = 1) + 
  stat_summary(fun.y = "median", color = "red", size = 3, geom = "point") + 
  ggtitle('median trend')

q_ph_jitter <- ggplot(data=rwd, aes(x=pH, y=quality)) + 
  geom_point(position = 'jitter') 
q_ph_box <- ggplot(data = rwd, aes(x = quality, y = pH)) +
  geom_boxplot(aes(group=quality)) + 
  scale_y_continuous(limits = c(quantile(rwd$pH, 0.05), quantile(rwd$pH, 0.95))) + 
  stat_summary(fun.y=median, color="red", geom="line", size = 1) + 
  stat_summary(fun.y = "median", color = "red", size = 3, geom = "point") + 
  ggtitle('median trend')

plot_grid(q_den_jitter, q_den_box, q_ph_jitter, q_ph_box, ncol=2)
```
...
```{r fig.width=9, fig.align='center', echo=FALSE, warning=FALSE}
ggplot(data=rwd, aes(x=fixed.acidity, y=citric.acid)) + 
  geom_point(aes(color=quality_fa, size=alcohol), alpha=0.5) + 
  scale_color_brewer(type = 'div', palette = 1) + 
  scale_x_continuous(limits = c(min(rwd$fixed.acidity), quantile(rwd$fixed.acidity, 0.95)))
```

The above plot shows relationships between four different variables, citric acid, fixed acidity, alcohol, and quality. This is pretty much the same to the second plot on this section, but alcohol is taken into consideration too. 

This plot somewhat demystifies why wines could have good quality even though low fixed acidity and low citric acid are present. They contain high level of alcohol. Similarily, wines with high quality and high citric acid could be rated as less quality when they contain less alcohol.

```{r fig.width=9, fig.align='center', echo=FALSE, warning=FALSE}
ggplot(data=rwd, aes(x=volatile.acidity, y=citric.acid)) + 
  geom_point(aes(color=quality_fa, size=alcohol), alpha=0.5) + 
  scale_color_brewer(type = 'div', palette = 1) + 
  scale_x_continuous(limits = c(min(rwd$volatile.acidity), quantile(rwd$volatile.acidity, 0.95)))
```

Fixed acidity and volatile acidity playes the opposite role to determine quality. Less volatile acidity can contribute wines to be better ones. 

Just like the previous plot, this plot gives a good understanding of the quality of wine. Even though less volatile and high citric acid are given, wines can be less quality when they contain less alcohol. Similarily, wines less volatile and less citric acid can have better quality when high alcohol is present.
...
### Plot Two
```{r fig.height=6, fig.width=9, fig.align='center', echo=FALSE, message=FALSE, warning=FALSE}
plot_two_1 <- ggplot(data=rwd, aes(x=sulphates, y=quality)) + 
  geom_point(position = 'jitter') + 
  scale_x_continuous(limits = c(0.4, quantile(rwd$sulphates, 0.95)))

plot_two_2 <- ggplot(rwd, aes(sulphates, color=quality_fa)) +
  geom_density()+ 
  scale_x_continuous(limits = c(0.4, quantile(rwd$sulphates, 0.95)))

plot_grid(plot_two_1, plot_two_2, ncol = 1)
```

### Description Two
Sulphates doesn't create relationship with other variables to determine wine quality. Rather, it only itself creates a strongly positive relationship with quality. As described, sulphates is a win additive meaning it is not situated naturally. Probably, that is why it doesn't have any relationship with other variables to assess wine quality. Additionally, high level of sulphates may be hard to be added to wines.

### Plot Three
```{r fig.height=8, fig.width=9, fig.align='center', echo=FALSE, warning=FALSE}
plot_three_1 <- ggplot(data=rwd, aes(x=fixed.acidity, y=citric.acid)) + 
  geom_point(aes(color=quality_fa, size=alcohol), alpha=0.5) + 
  scale_color_brewer(type = 'div', palette = 1)

plot_three_2 <- ggplot(data=rwd, aes(x=volatile.acidity, y=citric.acid)) + 
  geom_point(aes(color=quality_fa, size=alcohol), alpha=0.5) + 
  scale_color_brewer(type = 'div', palette = 1)
...
### Description Three
High fixed acidity, low volatile acidity, high citric acid, and high alcohol seem to be important to determine wine quality. Especially, alcohol level is important. 
...
There was clear two kinds of trend. The one is between the sulphates of a wine and its quality, and the other one is between fixed acidity, volatile acidity, citric acid, alcohol, and quality. I was surprised that lots of features, especially alcohol level, of a wine play an important role to determine its quality. Also, I was surprised that sulphates of a wine has a positive relationship with its quality because I though it could harm the wine. 
